...
export const ActiveBookingsPage = () => {
    ...
    const fetchActiveBookings = async () => {
        try {
            setLoading(true);
            const response = await api.get('/crm/bookings/active');
            if (response.data.success) {
                // Backend returns pre-formatted data matching our needs
                setActiveBookings(response.data.data.bookings.map(b => ({
                    ...b,
                    timeLeft: 'Ongoing' // Backend doesn't send timeLeft, defaulting (can be enhanced later)
                })));
            }
        } catch (error) {
    ...
    const filteredActive = activeBookings.filter(item =>
...
export const UpcomingBookingsPage = () => {
    ...
    const fetchUpcomingBookings = async () => {
        try {
            setLoading(true);
            const response = await api.get('/crm/bookings/upcoming');
            if (response.data.success) {
    ...
    const filteredUpcoming = upcomingBookings.filter(item =>
        item.vehicle.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.regNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.customer.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.phone.toLowerCase().includes(searchTerm.toLowerCase())
