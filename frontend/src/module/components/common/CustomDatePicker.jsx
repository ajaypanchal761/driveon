 ...
 */
const CustomDatePicker = ({ value, onChange, placeholder = 'Select date & time' }) => {
  ...
  const initialTime = selectedDate ? get12HourFormat(selectedDate.getHours()) : { hour: 10, period: 'am' };
  const [selectedHour, setSelectedHour] = useState(initialTime.hour);
  const [selectedMinute, setSelectedMinute] = useState(selectedDate ? selectedDate.getMinutes() : 30);
  ...
  const displayValue = selectedDate 
    ? `${selectedDate.toLocaleDateString()} ${String(selectedHour).padStart(2, '0')} : ${String(selectedMinute).padStart(2, '0')} ${selectedPeriod}`
    : placeholder;
    ...
    <div ref={datePickerRef} className="relative w-full">
        ...
        <>
          ...
          <div className="mb-2">
            ...
            <div className="grid grid-cols-7 gap-0.5 mb-1">
              {weekDays.map(day => (
                <div key={day} className="text-center text-[10px] font-medium" style={{ color: colors.textSecondary }}>
                  {day}
                </div>
              ))}
            </div>
              ...
              {days.map((date, idx) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      backgroundColor: isSelected ? colors.brandBlack : 'transparent',
                      color: isSelected ? colors.backgroundSecondary : (isCurrentMonth ? colors.textPrimary : colors.textTertiary),
                      border: isToday && !isSelected ? `1px solid ${colors.brandBlack}` : 'none',
                    }}
                  >
                    {date.getDate()}
                  </button>
          ...
          <div className="mb-2">
            <label className="block text-[10px] font-medium mb-1" style={{ color: colors.textPrimary }}>Time</label>
          ...
          {/* Action Buttons */}
          <div className="flex gap-1.5 mt-2 pt-2 border-t" style={{ borderColor: colors.borderForm }}>
            <button
              type="button"
              onClick={() => {
              ...
              style={{ color: colors.brandBlack, backgroundColor: 'transparent' }}
            >
              Clear
            </button>
            <button
      ...
      {isTimePickerOpen && (
        <>
          ...
          >
            <h3 className="text-[10px] font-bold mb-1.5 text-center" style={{ color: colors.textPrimary }}>Select Time</h3>
            
            {/* Time Selection */}
            <div className="flex items-center justify-center gap-1.5 mb-2">
              {/* Hour Selection */}
              <div className="flex flex-col items-center">
                <label className="text-[9px] font-semibold mb-0.5" style={{ color: colors.textSecondary }}>Hour</label>
              ...
              <span className="text-sm font-bold mt-5" style={{ color: colors.textPrimary }}>:</span>

              {/* Minute Selection */}
              <div className="flex flex-col items-center">
                <label className="text-[9px] font-semibold mb-0.5" style={{ color: colors.textSecondary }}>Minute</label>
                <div className="flex flex-col gap-0.5 max-h-24 overflow-y-auto">
              ...
              {/* AM/PM Selection */}
              <div className="flex flex-col items-center">
                <label className="text-[9px] font-semibold mb-0.5" style={{ color: colors.textSecondary }}>Period</label>
                <div className="flex flex-col gap-0.5">
                  {['am', 'pm'].map((period) => (
                    <button
                      ...
                      style={{
                        backgroundColor: selectedPeriod === period ? colors.backgroundTertiary : 'transparent',
                        color: selectedPeriod === period ? colors.backgroundSecondary : colors.textPrimary,
                      }}
                    >
                      {period.toUpperCase()}
                    </button>
